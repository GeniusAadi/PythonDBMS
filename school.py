...
def crt():
    ...
    for i in range(num_columns):
        column = input("Enter column name with its data type and key if needed: ")
...
def entr():
    ...
    mycursor.execute(f"INSERT INTO {table_name} VALUES ({value_str});")
...
def updt():
    ...
    for i in range(num_columns):
        ...
        value = input("Enter new value for the column: ")
        columns.append(column)
        values.append(value)
    condition_column = input("Enter column name for condition: ")
    condition_value = input("Enter value for condition: ")
    update_str = ", ".join([f"{col} = '{val}'" for col, val in zip(columns, values)])
    mycursor.execute(f"UPDATE {table_name} SET {update_str} WHERE {condition_column} = '{condition_value}';")
